...
from geo.tube import Tubeset,Geometry
...
def runFull(filename,mmpx):
    cv = Convert(mmpx=mmpx)
    P = ProfilePhoto(filename,mmpx=mmpx)
    G = Geometry(mmpx=mmpx)
    D = Display(mmpx=mmpx)
    ...
    G.T = Tubeset(mmpx=mmpx,rows=P.rows,cols=P.cols)
    ...
    D.plotLines(P.imW,lines,False,title="seat tube line detection")
    ...
    D.plotLines(P.imW,lines,False,title="head tube line detection")
    D.plotLines(P.imW,np.reshape(G.T.tubes['ht'].pt1+G.T.tubes['ht'].pt2,(1,1,4)).astype(int),False,title="head tube line detection",color=(255,0,0))
    ...
    D.plotFig(P.imW,False,title="head extend")
